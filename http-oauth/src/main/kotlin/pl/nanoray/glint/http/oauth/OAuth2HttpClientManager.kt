...
import pl.nanoray.glint.store.map.MutableMapStore
import pl.nanoray.glint.store.map.toMutableValueStoreForKey
import java.util.*
...
class StoreOAuth2SingleHttpClientManager<UserId, Token: OAuth2Token>(
	...
	private val store: MutableMapStore<UserId, Token>,
	...
	private val clients = mutableMapOf<UserId, OAuth2SingleHttpClient<UserId, Token>>().apply {
		for (userId in store.keys) {
			this[userId] = OAuth2SingleHttpClient(wrappedClient, userId, redirectManager, service, tokenParser, store.toMutableValueStoreForKey(userId))
		}
	}
	...
	private fun scheduleNextClientRefresh() {
		...
		val (client, token, refreshTokenExpiryTime) = clients.entries
			.mapNotNull { (userId, client) -> store[userId]?.let { client to it } }
	...
	override fun getHttpClient(userId: UserId): OAuth2SingleHttpClient<UserId, Token> {
		return clients.computeIfAbsent(userId) { OAuth2SingleHttpClient(wrappedClient, userId, redirectManager, service, tokenParser, store.toMutableValueStoreForKey(userId)) }
	}
}
...
class StoreOAuth2ObservableHttpClientManager<UserId, Token: OAuth2Token>(
	...
	private val clients = mutableMapOf<UserId, OAuth2ObservableHttpClient<UserId, Token>>().apply {
		for (userId in store.keys) {
			this[userId] = OAuth2ObservableHttpClient(wrappedClient, userId, redirectManager, service, tokenParser, store.toMutableValueStoreForKey(userId))
		}
	}
	...
	override fun getHttpClient(userId: UserId): OAuth2ObservableHttpClient<UserId, Token> {
		return clients.computeIfAbsent(userId) { OAuth2ObservableHttpClient(wrappedClient, userId, redirectManager, service, tokenParser, store.toMutableValueStoreForKey(userId)) }
	}
}